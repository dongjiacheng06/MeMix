    ...
    with torch.cuda.amp.autocast(enabled=not inference):
        ...
        def _detach_state(x):
            if isinstance(x, (tuple, list)):
                return tuple(item.detach() for item in x)
            return x.detach()

        init_state_feat = _detach_state(init_state_feat)
        init_mem = _detach_state(init_mem)

        for chunk_id in range((len(batch) - 1) // chunk_size + 1):
            preds = []
            chunk = []
            state_feat = _detach_state(state_feat)
            state_pos = state_pos.detach()
            mem = _detach_state(mem)
