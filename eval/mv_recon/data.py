...
class SevenScenes(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            color_files = [name for name in entries if "color" in name]
            if len(color_files) == 0:
                nested_path = osp.join(data_path, osp.basename(data_path))
                if osp.isdir(nested_path):
                    nested_entries = os.listdir(nested_path)
                    nested_color_files = [
                        name for name in nested_entries if "color" in name
                    ]
                    if nested_color_files:
                        data_path = nested_path
                        color_files = nested_color_files
                        entries = nested_entries
            if len(color_files) == 0:
                raise FileNotFoundError(
                    f"No 7scenes color frames found under {data_path}"
                )
            if any(name.endswith(".depth.proj.png") for name in entries):
                depth_suffix = ".depth.proj.png"
            elif any(name.endswith(".depth.png") for name in entries):
                depth_suffix = ".depth.png"
            else:
                raise FileNotFoundError(
                    f"No 7scenes depth frames found under {data_path}"
                )
            num_files = len(color_files)
            img_idxs = [f"{i:06d}" for i in range(num_files)]
            img_idxs = img_idxs[:: self.kf_every]
            if self.max_views > 0:
                img_idxs = img_idxs[: self.max_views]
        ...
        while len(imgs_idxs) > 0:
            ...
            impath = osp.join(data_path, f"frame-{im_idx}.color.png")
            depthpath = osp.join(data_path, f"frame-{im_idx}{depth_suffix}")
