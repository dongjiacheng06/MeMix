...
def bonn_rgb_dir(img_path, seq):
    return os.path.join(img_path, f"rgbd_bonn_{seq}", "rgb_full")


def bonn_depth_dir(depth_root, seq):
    return os.path.join(depth_root, f"rgbd_bonn_{seq}", "depth_full")


def bonn_gt_path(img_path, seq):
    return os.path.join(img_path, f"rgbd_bonn_{seq}", "groundtruth_full.txt")
...
dataset_metadata = {
    ...
    "kitti": {
        ...
        "depth_root": str(
            ...
            / "depth_selection"
            / "val_selection_cropped"
            / "groundtruth_depth_gathered_full"
        ),
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
        "gt_traj_func": lambda img_path, anno_path, seq: None,
    ...
    "bonn": {
        ...
        "depth_root": str(DATA_ROOT / "bonn" / "rgbd_bonn_dataset"),
        "mask_path": None,
    ...
    "sintel": {
        ...
        "depth_root": str(DATA_ROOT / "sintel" / "training" / "depth"),
...
def process_bonn(args, img_path):
    if args.full_seq:
        for directory in tqdm(sorted(glob.glob(f"{img_path}/*/"))):
            rgb_dir = os.path.join(directory, "rgb_full")
            filelist = sorted(glob.glob(f"{rgb_dir}/*.png"))
    ...
    for seq in tqdm(seq_list):
        rgb_dir = bonn_rgb_dir(img_path, seq)
        filelist = sorted(glob.glob(f"{rgb_dir}/*.png"))
