...
Example:
    python demo.py --model_path src/cut3r_512_dpt_4_64.pth \
        --seq_path /path/to/your_video.mp4 --model_variant cut_memix --device cuda --size 512
"""
...
framerate = 30
MODEL_VARIANTS = ("cut", "ttt", "ttsa", "cut_memix", "ttt_memix", "ttsa_memix")
...
def _ensure_runtime_dependencies():
    ...
    if missing:
        raise ModuleNotFoundError(
            "demo.py requires additional dependencies: "
            + ", ".join(missing)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to a local video file or image directory.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Model variant for the demo run. Choices: " + ", ".join(MODEL_VARIANTS),
    )
    parser.add_argument(
        "--frame_interval",
        type=int,
...
def run_inference(args):
    ...
    if hasattr(model, "set_variant"):
        model.set_variant(args.model_variant)
    else:
        raise AttributeError("Loaded model does not support variant selection")
...
def main():
    ...
    if not args.seq_path:
        print(
            "No inputs found. Please provide --seq_path pointing to an image folder or video."
        )
        return
    else:
        run_inference(args)
