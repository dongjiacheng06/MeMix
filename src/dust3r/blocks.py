...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, qpos, kpos, return_attn=False, attn_mask=None):
        B, Nq, C = query.shape
        Nk = key.shape[1]
        Nv = value.shape[1]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, y, xpos, ypos, return_attn=False, attn_mask=None):
        if return_attn:
            self_attn_output, self_attn = self.attn(
                self.norm1(x), xpos, return_attn=True
            )
            x = x + self.drop_path(self_attn_output)
            y_ = self.norm_y(y)
            cross_attn_output, cross_attn = self.cross_attn(
                self.norm2(x), y_, y_, xpos, ypos, return_attn=True, attn_mask=attn_mask
            )
        ...
        y_ = self.norm_y(y)
        x = x + self.drop_path(
            self.cross_attn(self.norm2(x), y_, y_, xpos, ypos, attn_mask=attn_mask)
        )
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        return x, y
